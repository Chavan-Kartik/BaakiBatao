...
pnpm eval:run        # generate + faults + engine + score; --count 200 for the full sweep,
                     # --profile degraded to route rows through OCR-style noise + the normaliser
pnpm eval:report     # the run, plus one row per fault with what the engine cited and why a miss was missed
pnpm eval:assert     # fail if either profile regressed against packages/eval/baseline*.json
pnpm eval:baseline   # deliberately re-record both baselines, with a note in the commit
...
| `@fc/rulepack` | All six IRDAI bright-line clauses encoded as data, validated, hashed. 73 line categories — the payable classes plus the IRDAI non-payable and subsumed lists — with the category ↔ clause contract asserted at load time. |
| `@fc/normalise` | Tier 1 of the cascade: the lexicon built from the rulepack's aliases, exact and trigram-fuzzy, with a single calibrated threshold. Pure; tiers 2–3 plug in through an `Escalation` hook. |
...
Two profiles run over the same 200 seeded packs. Every figure is regenerated into
[`docs/evaluation.md`](docs/evaluation.md) by `pnpm eval:doc`; the ones below are from there.

| | clean | degraded |
|---|---:|---:|
| injected faults detected | 178 / 178 | 63 / 178 |
| lawful control packs disputed | 0 | 0 |
| misses attributable to the engine | 0 | 0 |

**Clean** is rows exactly as generated, categories by construction. It measures the
waterfall alone: precision and recall **1.00** per clause, and no lawful deduction in the
control set disputed. Those are the two failure modes that matter — a missed unlawful
deduction, and a lawful one called unlawful — and the regression gate holds both.

**Degraded** is the same packs with descriptions and digits perturbed the way OCR perturbs
them, sheet rows occasionally missing, and categories assigned by the real tier-1 normaliser.
It measures the system as far as it exists without AWS. Detection falls to 35%, and the
report says exactly where: every miss is a line the normalisation gate excluded or a sheet
row that no longer matched — none is the engine citing the wrong clause, and still no
lawful cut is disputed. The calibration sweep in the same document shows the §14 default
threshold escalating 51% of lines where 0.52 escalates 13% at 99.98% accuracy; at that
operating point detection under degradation is 64%. The default is recorded, not changed.
